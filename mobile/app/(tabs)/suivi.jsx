    ...
    >
      <View className="flex-row items-center justify-between mb-2">
        ...
        <View className="flex-row items-center bg-fuchsia-50 rounded-lg px-2 py-1" style={{ gap: 4 }}>
          ...
          <Text className="text-[9px] font-bold text-fuchsia-600 uppercase">Live</Text>
        </View>
      </View>
...
export default function SuiviScreen() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      triggerToast("Impossible de charger vos suivis.", "error");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleAccept = async (inv) => {
    ...
    try {
      await acceptSuiviInvitation(inv.id);
  ...
  const handleDecline = async (inv) => {
    setRespondingId(inv.id);
    try {
      await deleteSuiviInvitation(inv.id);
  ...
  const sections = [];
  if (invitations.length > 0) sections.push({ type: "invitations", data: invitations });
    ...
    <SafeAreaView className="flex-1 bg-slate-50" edges={["top"]}>
      ...
      <View className="px-4 pt-5 pb-2 flex-row items-center justify-between">
        <View className="flex-1 pr-3">
          <Text className="text-xl font-extrabold text-slate-900">Amis & Live</Text>
          <Text className="text-xs text-slate-500 mt-1">
            Repérez-vous en direct avec vos amis.
          </Text>
        </View>
        <Pressable
          onPress={() => router.push("/modals/create-suivi")}
          className="bg-fuchsia-600 rounded-xl px-3.5 py-2.5 flex-row items-center active:opacity-80"
          style={{ gap: 6 }}
        >
